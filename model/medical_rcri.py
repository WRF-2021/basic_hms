...
class medical_rcri(models.Model):
    ...
    rcri_date = fields.Datetime('RCRI Fecha')
    rcri_physician_id = fields.Many2one('medical.physician','Profesional de la salud')
    rcri_high_risk_surgery = fields.Boolean('Cirugía de alto riesgo')
    rcri_ischemic_history = fields.Boolean('Historia de cardiopatía isquémica')
    rcri_congestive_history = fields.Boolean('Historia de cardiopatía congestiva')
    rcri_diabetes_history = fields.Boolean('Diabetes preoperatoria')
    rcri_cerebrov_history = fields.Boolean('Historia de enfermedad cerebrovascular')
    rcri_kidney_history = fields.Boolean('Enfermedad renal preoperatoria')
    rcri_total = fields.Integer('Puntuación', compute='rcri_total_count')
